...
class _RaceControlScreenState extends State<RaceControlScreen>
  ...
  Future<void> _updateFlag(RaceFlag flag,
      {RaceSession? session, RaceEvent? currentEvent}) async {
    try {
      final raceId = currentEvent?.trackId ?? widget.event.trackId;
      final raceDoc = await _firestoreService.getRace(raceId);
      if (raceDoc == null) {
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(
              content: Text(
                'Track not found for this event. Edit the event and select a valid track.',
              ),
            ),
          );
        }
      ...
      await _firestoreService.updateRaceFlag(raceId, flag);
      ...
      if (passingFlags.isNotEmpty) {
        ...
        await _firestoreService.addPassing(
          flagPassing,
            ...
            if (event is KeyDownEvent) {
              if (event.logicalKey == LogicalKeyboardKey.f5) {
                ...
                if (displaySession != null)
                  _showManualEntryDialog(eventData.trackId,
