...
class _EventRegistrationScreenState extends State<EventRegistrationScreen> {
  ...
  Future<List<Map<String, dynamic>>> _loadTrackCheckpoints() async {
    final event = _currentEvent;
    if (event == null || event.trackId.isEmpty) return const [];
    ...
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        content: TextField(
          ...
          decoration:
              const InputDecoration(labelText: 'Group Name (e.g. Kids, Pro)'),
        ),
        actions: [
          TextButton(
      ...
      builder: (context, constraints) {
        ...
        return Scaffold(
              ...
              IconButton(
                ...
                onPressed: () {
                  Navigator.push(
                    ...
                    MaterialPageRoute(
                        builder: (context) =>
                            RaceControlScreen(event: _currentEvent!)),
                  );
                },
              ),
              IconButton(
                ...
                onPressed: () async {
                  await Navigator.push(
                    ...
                    MaterialPageRoute(
                      builder: (context) =>
                          CreateEventScreen(event: _currentEvent),
                    ),
                  );
                  _refreshEvent();
                },
          ...
          body: isMobile
              ...
              : _buildDesktopLayout(),
          floatingActionButton: (!showDetails || !isMobile)
  ...
  Widget _buildMobileLayout(bool showDetails) {
    if (showDetails) {
      return _buildGroupDetails(
          _currentEvent!.groups.firstWhere((g) => g.id == _selectedGroupId),
          isMobile: true);
    } else {
      ...
      children: [
          ...
          child: Column(
            children: [
              Container(
                ...
                child: Center(
                    child: Text('Groups',
                        style: SpeedDataTheme.textTheme.headlineMedium)),
              ),
        ...
        Expanded(
          child: _selectedGroupId == null
              ? const Center(
                  child:
                      Text('Select a Group to manage Sessions and Competitors'))
              : _buildGroupDetails(_currentEvent!.groups
                  .firstWhere((g) => g.id == _selectedGroupId)),
        ),
      ],
    );
        ...
        return ListTile(
          title: Text(group.name,
              style: TextStyle(
                  ...
                  color:
                      isSelected ? Colors.white : SpeedDataTheme.textPrimary)),
          selected: isSelected,
          ...
          trailing: IconButton(
            icon: const Icon(Icons.delete_outline,
                size: 20, color: SpeedDataTheme.textSecondary),
  ...
  void _confirmDeleteGroup(int index) {
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        content: const Text(
            'This will delete the group and all its sessions and competitors. This action cannot be undone.'),
        actions: [
          TextButton(
              onPressed: () => Navigator.pop(context),
              child: const Text('Cancel')),
      ...
      builder: (context) => AlertDialog(
        ...
        actions: [
          ...
          TextButton(
            ...
            style:
                TextButton.styleFrom(foregroundColor: SpeedDataTheme.flagRed),
            child: const Text('Delete'),
          ),
        ],
      ),
  ...
  void _confirmDeleteSession(RaceSession session) {
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        content: const Text(
            'This will delete the session and its results. This action cannot be undone.'),
        actions: [
          TextButton(
              onPressed: () => Navigator.pop(context),
              child: const Text('Cancel')),
              ...
              ToggleButtons(
                ...
                children: const [
                  Padding(
                      ...
                      child: Text('Running Order')), // Schedule
                  Padding(
                      padding: EdgeInsets.symmetric(horizontal: 16),
  ...
  Widget _buildScheduleView(RaceGroup group) {
    final groupSessions =
        _currentEvent!.sessions.where((s) => s.groupId == group.id).toList();
            ...
            itemBuilder: (context, index) {
              ...
              return Card(
                ...
                child: ListTile(
                  ...
                  title: Text(session.name.isNotEmpty
                      ? session.name
                ...
                children: [
                  ...
                  OutlinedButton.icon(
                    icon: const Icon(Icons.swap_horiz,
                        color: SpeedDataTheme.textPrimary),
                    label: const Text('Transfer',
                        style: TextStyle(color: SpeedDataTheme.textPrimary)),
  ...
  Future<void> _showAddSessionDialog(RaceGroup group) async {
    final trackCheckpoints = await _loadTrackCheckpoints();
    if (!mounted) return;
  ...
  Future<void> _editSession(RaceSession session) async {
    final trackCheckpoints = await _loadTrackCheckpoints();
    if (!mounted) return;
      ...
      MaterialPageRoute(
        builder: (context) => SessionSettingsScreen(
          ...
          onSave: (updatedSession) {
            setState(() {
              final index = _currentEvent!.sessions
    ...
    Navigator.push(
      ...
      MaterialPageRoute(
        builder: (context) => CompetitorSettingsScreen(
          groupId: group.id,
          onSave: (newComp) =>
              _firestoreService.addCompetitor(_currentEvent!.id, newComp),
        ),
      ),
    );
  }
  ...
  void _showTransferCompetitorsDialog(RaceGroup currentGroup) async {
    final allCompetitors =
        await _firestoreService.getCompetitors(_currentEvent!.id);
    final groupCompetitors =
        allCompetitors.where((c) => c.groupId == currentGroup.id).toList();

    if (groupCompetitors.isEmpty) {
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
          content: Text('No competitors in this group to transfer.')));
      return;
    }

    final otherGroups =
        _currentEvent!.groups.where((g) => g.id != currentGroup.id).toList();
    if (otherGroups.isEmpty) {
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('No other groups to transfer to.')));
      return;
    }

    Set<String> selectedIds =
        Set<String>.from(groupCompetitors.map((c) => c.id));
          ...
          return AlertDialog(
            ...
            content: SizedBox(
              ...
              child: Column(
                ...
                children: [
                  Text('Transferring from ${currentGroup.name}',
                    ...
                    children: [
                      TextButton(
                          onPressed: () => setState(() => selectedIds
                              .addAll(groupCompetitors.map((c) => c.id))),
            ...
            actions: [
              TextButton(
                  ...
                  child: const Text('Cancel')),
              ElevatedButton(
                onPressed: selectedIds.isEmpty || targetGroupId == null
                    ? null
                    : () {
  ...
  void _performTransfer(
      List<Competitor> competitors, String targetGroupId, bool isCopy) async {
    ...
    ScaffoldMessenger.of(context).showSnackBar(SnackBar(
        content: Text(
            'Successfully ${isCopy ? 'copied' : 'moved'} ${competitors.length} competitors')));
  }
      ...
      MaterialPageRoute(
        builder: (context) => CompetitorSettingsScreen(
          ...
          onSave: (updatedCompetitor) {
            _firestoreService.addCompetitor(
                _currentEvent!.id, updatedCompetitor);
          },
        ),
      ),
    );
